...
const ToDo = () => {
    ...
    const handleChange = (event) => {
        setEditId({ ...editId, [event.target.name]: event.target.value })
    }
    return (
        <>
            {
                        ...
                        <div className='h-full w-96 mx-auto flex self-center justify-center items-center'>
                            <div className=' border-t-4 flex justify-center flex-col items-center border-brand flex-1 p-4 bg-neutral-800'>
                                ...
                                <div className='flex flex-row gap-4 self-center'>
                                    <button className='text-white border-2 border-brand w-20 rounded-md  p-1' onClick={() => { handleDelete(deleteId); toggleOpen(); toggleButton() }}>Yes</button>
                    ...
                    <div className='flex flex-1 flex-col items-center'>
                        <div className='h-32'>
                            <form className="flex max-w-md flex-row gap-4" onSubmit={handleSubmit}>
                                {editId ? <>
                                    <div>
                                        <input placeholder='Input' name='title' type="text" value={editId?.title} className='w-full border-2 border-brand mb-2 text-white bg-brand rounded-md p-3  '
                                            onChange={handleChange} 
                                        />
                                        <input placeholder='Input' name='description' type="text" value={editId?.description}  className='w-full border-2 border-brand text-white bg-brand rounded-md p-3  '
                                            onChange={handleChange} 
                                        />
                                    </div>
                                    <div>
                                        <button className='w-32 border-2 border-brand h-full text-white rounded-md'>
                                            <span className='inline-block  -mt-4 text-brand text-7xl h-4'>UPDATE</span>
                                        </button>
                                    </div>
                                </>  : <>
                                    <div>
                                        ...
                                        i
                                            ...
                                            {showButton && (
                                                <div className='flex flex-row gap-4'>
                                                    <div onClick={() => { setDeleteId(todo?.id);; setEditId(todo) }} className='border-2 border-brand w-12 flex justify-center items-center rounded-md h-12 hover:border-amber-400 background-design'>
                                                        <EditIcon />
                                                    </div>
                                                    <div onClick={() => { setDeleteId(todo?.id); toggleOpen(); }} className='border-2 border-brand w-12 flex justify-center items-center rounded-md h-12 hover:border-amber-400 delete-color '>
                                                        X
                                                    </div>
                                                </div>
                                            )}
